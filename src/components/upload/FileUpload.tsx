...
import { useFileStore, FileWithContent } from "@/store/useFileStore";
import { v4 as uuidv4 } from "uuid";
import { UPLOAD_CONFIG } from "@/config/upload";
import { calculateHash } from "@/utils/hash";

// 添加类型定义
type FileType = keyof typeof UPLOAD_CONFIG.allowedTypes;
...
export default function FileUpload() {
  ...
  const validateFile = (file: File) => {
    ...
    const fileType = Object.entries(UPLOAD_CONFIG.allowedTypes).find(
    ...
    if (file.size > UPLOAD_CONFIG.allowedTypes[fileType].maxSize) {
      throw new Error(
    ...
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        const content = await new Promise<string>((resolve, reject) => {
          ...
          reader.onerror = () =>
            reject(new Error(UPLOAD_CONFIG.errorMessages.readError));
        ...
        const newFile: FileWithContent = {
          ...
          contentHash: await calculateHash(content),
        };
      ...
      >
        ...
        <VStack spacing={4}>
          <FontAwesomeIcon
            ...
            size="3x"
            color={
              isDragReject ? "#F56565" : isDragActive ? "#4299E1" : "#A0AEC0"
            }
          />
          <Text
            textAlign="center"
            color={
              isDragReject ? "red.500" : isDragActive ? "blue.400" : "gray.500"
            }
          >
            {isDragReject
              ? "不支持的文件类型"
